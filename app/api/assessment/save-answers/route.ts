...
export async function POST(req: Request) {
  try {
    ...
    // Verify session & payment status from DB if available
    try {
      ...
      if (session && !payment && session.status !== 'paid' && session.status !== 'completed') {
        return NextResponse.json({ success: false, error: 'Unpaid assessment session. Payment required.' }, { status: 403 });
      }
    } catch (dbErr) {
      console.warn('Database verification skipped/failed in save-answers:', dbErr);
    }
    ...
    // Save answers & report record to database if available
    try {
      ...
      console.warn('Database save skipped/failed in save-answers:', dbErr);
    }
